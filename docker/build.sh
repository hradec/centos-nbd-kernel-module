...
[ $? -ne 0 ] && echo ERRO Downloading $kernel!! && break
...
sudo tar xvf linux-*.tar.xz -C /usr/src/kernels/ || \
...
make prepare && \
make scripts
[ $? -ne 0 ] && echo ERRO preparing kernel!! && break
...
make CONFIG_BLK_DEV_NBD=m M=drivers/block )
[ $? -ne 0 ] && echo ERRO building kernel module!! && break
